...
## Current focus
**Closed the discovery loop (2026-05-29).** The customs-data wedge is live on REAL data AND usable end-to-end in the product. Build → score → reach is now a single flow on the Buyer-Match page: type a product → live customs discovery → ranked real US buyers → enrich to full profiles → find contacts. See the top two Recent decisions entries. **Open follow-ups:** (1) **contact hit-rate** — website-based contact scraping finds nothing on JS storefronts / sites without crawlable contacts, and customs profiles often lack a website at all; improving this needs better crawl tuning or a non-website contact source. (2) **non-US data** is a business decision, not built — ImportYeti is US-import-only, so all discoverable buyers are US importers; serving exporters targeting other markets needs EU/India/etc. customs sources (deliberately deferred). (3) **Apify cost controls** (per-org budget caps/rate limits) deferred per owner. (4) confirm the authed `/dashboard/matches` + dossier visuals in a signed-in browser on prod (can't auth locally/in-agent). (5) buyer-fit weights in [buyerFit.ts](src/lib/scoring/buyerFit.ts) are a v1 to tune on a pilot user's feedback.
