...
## Current focus
**Prove the moat is live (2026-05-29).** The customs-data wedge is now demonstrably live on REAL data, not seeds — see the top Recent decisions entry. Two wedge bugs were found and fixed against live ImportYeti runs, and 10 real, correctly-classified US importer buyers were ingested through the production pipeline (Kalustyan 1,888 shipments / fit 92, Olam Farming 7,037, McCormick variants…). **Next slice (in progress):** the product→buyer **discovery engine** — product keyword search → HS-code relevance filter on suppliers (drop "King Pepper = sauces" / Black&Decker keyword noise) → harvest each relevant supplier's US `topTradingPartners` as candidate buyers → optional 2nd `type=company` enrichment pass → ingest as scored Importer buyers. This is the scalable "find buyers who import what you sell" engine. Prior follow-ups still open: confirm dossier/matches visuals on prod; buyer-fit weights/taxonomy in [buyerFit.ts](src/lib/scoring/buyerFit.ts) are a v1 to tune on feedback.
...
- **2026-05-29** — **Proved the moat is live on real customs data + fixed two wedge bugs** (commits `4ae3d87`, `44ca0b6`, `e3e539d`, `70ab9a2`; deployed prod). Goal: stop *claiming* a customs-data moat backed mostly by SQL seeds and actually run the Apify→ImportYeti→enrich→score pipeline end-to-end on genuine data. Two bugs surfaced only by running the real actor and inspecting its output:
  ...
  - **Next**: the product→buyer **discovery engine** (see Current focus) — the scalable wedge that turns a product search into a ranked buyer list via HS-code-filtered supplier→trading-partner harvesting.
