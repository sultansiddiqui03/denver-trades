...
## Current focus
**Prove the moat is live (2026-05-29).** The customs-data wedge is now demonstrably live on REAL data, not seeds — see the top Recent decisions entry. Two wedge bugs were found and fixed against live ImportYeti runs; 10 real, correctly-classified US importer buyers were ingested via the production pipeline (Kalustyan 1,888 shipments / fit 92, Olam Farming 7,037, McCormick variants…); and the product→buyer **discovery engine** shipped and is proven live ("black pepper" → 7 real US buyers harvested from HS-filtered suppliers). **Open follow-ups (next slices):** (1) **enrich discovered leads** — re-run the `…~buyers` (type=company) lookup on discovery winners to lift them from thin ~25 scores to full profiles; (2) **UI surface** for discovery (admin endpoint only today) — likely a "Find buyers of <product>" action wired into the Lead Scraper / Buyer-Match pages; (3) fuzzy-dedupe ImportYeti name variants; (4) confirm dossier/matches visuals on prod; (5) buyer-fit weights/taxonomy in [buyerFit.ts](src/lib/scoring/buyerFit.ts) are a v1 to tune on feedback.
